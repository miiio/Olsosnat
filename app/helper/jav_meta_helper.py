...
EXPIRE_TIMESTAMP = 365 * 24 * 3600


@singleton
class JavMetaHelper(object):
    """
    {
        "id": '',
        "title": '',
    ...
    def init_config(self):
        ...
        if laboratory:
            self._jav_cache_expire = laboratory.get("jav_cache_expire")
        self._meta_path = os.path.join(Config().get_config_path(), 'jav.dat')
    ...
    def get_meta_data_by_key(self, key):
        ...
        with lock:
            ...
            if info:
                ...
                elif expire and self._jav_cache_expire:
    ...
    def dump_meta_data(self, search, page, num):
        ...
        with lock:
            search_metas = [(k, v, str(k))
                for k, v in self._meta_data.items() if search.lower() in k.lower() and v.get("id")]
    ...
    def delete_meta_data_by_JAVid(self, JAVid):
        """
        清空对应JAVID的所有缓存记录，以强制更新JAV中最新的数据
        """
        for key in list(self._meta_data):
            if str(self._meta_data.get(key, {}).get("id")) == str(JAVid):
                with lock:
                    self._meta_data.pop(key)

    def delete_unknown_meta(self):
        """
        清除未识别的缓存记录，以便重新检索JAV
        """
        for key in list(self._meta_data):
            if str(self._meta_data.get(key, {}).get("id")) is None:
                with lock:
                    self._meta_data.pop(key)
